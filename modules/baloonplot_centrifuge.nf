...
cat *pavian_report_filtered.csv | grep -w "P" | awk -F"\t" '$1 > 0.02' | cut -f6 | sort | uniq | tr -d " " > .organismlist.dataR
...
cat .organismlist.dataR | while IFS= read -r orgname ; do 
	...
	for x in *pavian_report_filtered.csv ; do
		...
		readcount=$(grep -w "P" ${x}| cut -f1,6 | grep -w "$orgname" | cut -f1 | awk -F, '$1 > 0.02'| tr -d  "\n" | tr -d " ")
		if [ -z "$readcount" ]
			then
...
cat *pavian_report_filtered.csv | grep -w "C" | awk -F"\t" '$1 > 0.02' | cut -f6 | sort | uniq | tr -d " " > .organismlist.dataR
...
cat .organismlist.dataR | while IFS= read -r orgname ; do 
	...
	for x in *pavian_report_filtered.csv ; do
		# if grep true else "NA,"		
		readcount=$(grep -w "C" ${x}| cut -f1,6 | grep -w "$orgname" | cut -f1 | awk -F, '$1 > 0.02'| tr -d  "\n" | tr -d " ")
		if [ -z "$readcount" ]
			then
      			printf "NA," >> class.csv
			else
...
cat *pavian_report_filtered.csv | grep -w "G" | awk -F"\t" '$1 > 0.02' | cut -f6 | sort | uniq | cut -f15,16 | tr -d " " > .organismlist.dataR
...
cat .organismlist.dataR | while IFS= read -r orgname ; do 
	...
	for x in *pavian_report_filtered.csv ; do
		# if grep true else "NA,"		
		readcount=$(grep -w "G" ${x}| cut -f1,6 | grep -w "$orgname" | cut -f1 | awk -F, '$1 > 0.02'| tr -d  "\n" | tr -d " ")
		if [ -z "$readcount" ]
			then
      			printf "NA," >> genus.csv
			else
...
cat *pavian_report_filtered.csv | grep -w "S" | awk -F"\t" '$1 > 0.02' | cut -f6 | sort | uniq | cut -f15,16 | awk '{$1=$1;print}' > .organismlist.dataR
...
cat .organismlist.dataR | while IFS= read -r orgname ; do 
	...
	for x in *pavian_report_filtered.csv ; do
		# if grep true else "NA,"		
		readcount=$(grep -w "S" ${x}| cut -f1,6 | grep -w "$orgname" | cut -f1 | awk -F, '$1 > 0.02'| tr -d  "\n" | tr -d " ")
		if [ -z "$readcount" ]
			then
